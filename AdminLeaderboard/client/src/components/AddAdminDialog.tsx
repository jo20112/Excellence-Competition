...
interface AddAdminDialogProps {
  ...
  onAdd: (name: string, adminId?: string, avatarUrl?: string) => void;
}
...
export default function AddAdminDialog({ open, onOpenChange, onAdd }: AddAdminDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim()) {
      onAdd(name.trim(), adminId.trim() || undefined, avatarUrl || undefined);
      setName("");
      setAdminId("");
      setAvatarUrl("");
      onOpenChange(false);
      ...
      <DialogContent className="sm:max-w-md" data-testid="dialog-add-admin">
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="adminId" className="text-right block">
