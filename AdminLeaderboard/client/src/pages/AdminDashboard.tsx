...
import { fetchAdmins, addAdmin, updateAdmin, updateAdminPoints, Admin } from "@/lib/adminService";
...
export default function AdminDashboard() {
  ...
  const handleAddAdmin = async (name: string, adminId?: string, avatarUrl?: string) => {
    try {
      const initials = name.split(" ").map(n => n[0]).join("");
      await addAdmin(name, initials, adminId, avatarUrl);
      await loadAdmins();
      toast({
  ...
  const handleUpdateAdmin = async (id: string, name: string, adminId?: string, avatarUrl?: string) => {
    try {
      const initials = name.split(" ").map(n => n[0]).join("");
      await updateAdmin(id, name, initials, adminId, avatarUrl);
      await loadAdmins();
      toast({
        title: "تم تحديث بيانات المشرف",
        description: `تم تحديث بيانات ${name} بنجاح`,
      });
    } catch (error: any) {
      toast({
        title: "خطأ في تحديث البيانات",
        description: error.message || "حدث خطأ أثناء تحديث بيانات المشرف",
        variant: "destructive",
      });
    }
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div>
          ...
          <LeaderboardTable admins={admins} onAction={handleAction} onEdit={handleEditClick} />
        </div>
